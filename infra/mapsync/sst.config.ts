...
export default $config({
  app(input) {
    return {
      name: 'minecraft-ondemand-mapsync',
      removal: 'retain',
      home: 'aws',
      providers: {
        aws: {
  ...
  async run() {
    ...
    const repo = new aws.ecr.Repository('MapSyncRepository', {
      name: $app.name,
      imageTagMutability: 'IMMUTABLE',
    });

    new aws.ecr.RepositoryPolicy('MapSyncRepositoryPolicy', {
    ...
    const lifecyclePolicy = new aws.ecr.LifecyclePolicy('MapSyncLifecyclePolicy', {
      repository: repo.name,
      policy: {
        rules: [
          {
    ...
    if (process.env.GITHUB_REF_TYPE === 'tag' && process.env.GITHUB_REF_NAME) {
      dockerTags.push($interpolate`${repo.repositoryUrl}:${process.env.GITHUB_REF_NAME.replace('mapsync', '')}`);
      cacheFrom.push({
        registry: {
    ...
    const image = new dockerBuild.Image('MapSyncImage', {
      tags: dockerTags,
      context: {
        location: '../../../../packages/mapsync',
      },
